...
#listin = 'files.txt' 
normed_training_data = 'normed_data_4labels.pickle'
LARGE = 200.
#listin = 'test.txt' 
#a = open(listin, 'r')
...
def get_normalized_training_data(testfile_in):
  ...
  fn = testfile_in 
  T_est,g_est,feh_est,alpha_est= np.loadtxt(fn, usecols = (3,4,5,7), unpack =1) 
...
#def infer_labels(fn_pickle,testdata, fout_pickle, weak_lower=0.935,weak_upper=0.98):
    ...
    for jj in range(0,nstars):
      ...
      x10,x11,x12,x13,x14,x15,x16,x17,x18 = coeffs[:,10], coeffs[:,11], coeffs[:,12], coeffs[:,13], coeffs[:,14], coeffs[:,15], coeffs[:,16] ,coeffs[:,17], coeffs[:,18] 
      Cinv = 1. / (ysigma ** 2 + scatters ** 2)
      Params,covs = nonlinear_invert(f, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10,x11,x12,x13,x14,x15,x16,x17,x18,1/Cinv**0.5 ) 
      Params = Params+offsets 
      coeffs_slice = coeffs[:,-18:]
      MCM_rotate = np.dot(coeffs_slice.T, Cinv[:,None] * coeffs_slice)
      Params_all[jj,:] = Params 
      MCM_rotate_all[jj,:,:] = MCM_rotate 
      covs_all[jj,:,:] = covs
    filein = fout_pickle.split('_4labels_tags') [0] 
    if logical_or(filein == 'self_2nd_order_4labels', filein[0:4] == 'self'):
...
def func(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, a, b, c, d):
    f = (0
         + x1*a
         + x2*b
         + x3*c
...
def nonlinear_invert(f, x1, x2, x3, x4, x5, x6, x7, x8, x9 ,x10, x11, x12, x13, x14, x15, x16, x17, x18, sigmavals):
    def wrapped_func(observation_points, a, b, c, d):
        x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18  = observation_points
        return func(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18,  a, b, c, d)

    xdata = np.vstack([x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18])
...
fpickle2 = "coeffs_2nd_order_4labels.pickle"
...
self_flag = 1 
if self_flag == 2:
  field = "self_2nd_order_4labels_"
  file_in = open(normed_training_data, 'r') 
  testdataall, metaall, labels,  cluster_name,ids = pickle.load(file_in)
  file_in.close() 
  testmetaall, inv_covars = infer_labels_nonlinear("coeffs_2nd_order_4labels.pickle", testdataall, ids, field+"4labels_tags.pickle",-10.950,10.99) 
...
if self_flag == 1:
  ...
  testmetaall, inv_covars = infer_labels_nonlinear("coeffs_2nd_order_4labels.pickle", testdataall, ids, "training_LAMOST_ordered_notintraining_4labels_tags.pickle",0.00,1.40) 
